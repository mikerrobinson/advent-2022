...
class Monkey {
  ...
  processRound() {
    for (let i = 0; i < this.items.length; i++) {
      ...
      this.items[i] = this.items[i] % FACTOR;
...
// had to look this up - multiplying the "test" values together gives
// a number we can modulo the increasing large worry value by to keep it low
const FACTOR = monkeys.reduce( (accumulator, monkey) => accumulator * monkey.test, 1);
...
for (let round = 1; round <= MAX_ROUNDS; round++) {
  ...
  if (round == 1 || round == 20 || round % 1000 == 0) {
    ...
    for (let i = 0; i<monkeys.length; i++) {
      console.log(`Monkey ${i} inspected items ${monkeys[i].inspections} times.`);
    }
    console.log();
  }
}
